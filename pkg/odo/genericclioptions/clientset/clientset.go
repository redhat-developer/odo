...
var subdeps map[string][]string = map[string][]string{
	...
	DEPLOY:           {KUBERNETES},
	DEV:              {KUBERNETES, PORT_FORWARD, PREFERENCE, STATE, WATCH},
...
func Fetch(command *cobra.Command) (*Clientset, error) {
	...
	if isDefined(command, DEV) {
		dep.DevClient = dev.NewDevClient(dep.KubernetesClient, dep.PreferenceClient, dep.PortForwardClient, dep.WatchClient, dep.StateClient)
	}
