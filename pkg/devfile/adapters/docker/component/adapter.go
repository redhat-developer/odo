...
func (a Adapter) Push(path string, ignoredFiles []string, forceBuild bool, globExps []string) (err error) {
	componentExists := utils.DockerComponentExists(a.Client, a.ComponentName)
...
func (a Adapter) createOrUpdateComponent(componentExists bool) (err error) {
	...
	volumeNameToDockerVolumeName := make(map[string]string)
	...
	for _, volumes := range componentAliasToVolumes {
		for _, vol := range volumes {
			if _, ok := processedVolumes[*vol.Name]; !ok {
				...
				// Generate the Docker volume Names
				glog.V(3).Infof("Generating Docker volume name for %v", *vol.Name)
				generatedPVCName, err := storage.GeneratePVCNameFromDevfileVol(*vol.Name, componentName)
				if err != nil {
					return err
				}

				// Check if we have an existing Docker volume with the labels, overwrite the generated name with the existing name if present
				existingName, err := storage.GetExistingPVC(&a.Client, *vol.Name, componentName)
				if err != nil {
					return err
				}
				if len(existingName) > 0 {
					glog.V(3).Infof("Found an existing Docker volume for %v, Docker volume %v will be re-used", *vol.Name, existingName)
					generatedPVCName = existingName
				}
				...
				volumeNameToDockerVolumeName[*vol.Name] = generatedPVCName
			}
		}
	}
