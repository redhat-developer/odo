...
func List(client *occlient.Client, applicationName string) (ServiceList, error) {
	...
	for _, elem := range serviceInstanceList {
		...
		svc := GetMachineReadableFormat(elem.Labels[componentlabels.ComponentLabel], elem.Labels[componentlabels.ComponentTypeLabel], status)
		services = append(services, svc)
	}

	return GetMachineReadableFormatForList(services), nil
}
...
func ListWithDetailedStatus(client *occlient.Client, applicationName string) (ServiceList, error) {
	...
	for i, service := range services.Items {
		for _, secret := range secrets {
			if secret.Name == service.Name {
				...
				services.Items[i].Status.Message = provisionedAndBoundStatus
	...
	return GetMachineReadableFormatForList(services.Items), nil
}
	...
	for _, dc := range dcs {
		...
		for _, env := range dc.Spec.Template.Spec.Containers[0].EnvFrom {
			if env.SecretRef.Name == secretName {
				services[index].Status.Message = provisionedAndLinkedStatus
			}
...
// GetMachineReadableFormat gives machine readable service definition
func GetMachineReadableFormat(name, serviceType, status string) Service {
	return Service{
		TypeMeta:   metav1.TypeMeta{Kind: "service", APIVersion: "odo.openshift.io/v1alpha1"},
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec:       ServiceSpec{ServiceType: serviceType},
		Status:     ServiceStatus{Message: status},
	}

}

// GetMachineReadableFormatForList gives machine readable service list definition
func GetMachineReadableFormatForList(services []Service) ServiceList {
	return ServiceList{
