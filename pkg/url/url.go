...
func Create(client *occlient.Client, kClient *kclient.Client, parameters CreateParameters, isRouteSupported bool) (string, error) {
	...
	if parameters.urlKind == envinfo.INGRESS && kClient != nil {
		if parameters.host == "" {
...
func Push(client *occlient.Client, kClient *kclient.Client, parameters PushParameters) error {
	...
	for urlName, urlInfo := range urlLOCAL {
		...
		if !ok {
			...
			host, err := Create(client, kClient, createParameters, parameters.IsRouteSupported)
			if err != nil {
				return err
			}
