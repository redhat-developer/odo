...
func TestGetS2IPathsFromBuilderImg(t *testing.T) {
	tests := []struct {
		...
		wantProtocol      string
		wantPath          string
		wantSrcBinPath    string
		wantDeploymentDir string
		wantErr           bool
	}{
		{
			...
			wantProtocol:      "image://",
			wantSrcBinPath:    "/opt/app-root/src",
			wantDeploymentDir: "/opt/app-root/src",
		...
		},
		{
			...
			wantProtocol:      "image://",
			wantSrcBinPath:    "/tmp",
			wantDeploymentDir: "/deployments",
			wantPath:          "/usr/local/s2i",
			wantErr:           false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotProtocol, gotPath, gotSrcBinPath, gotSrcDeploymentsDir, err := GetS2IPathsFromBuilderImg(tt.imageStreamImage)
			...
			if tt.wantSrcBinPath != gotSrcBinPath {
				t.Errorf("Tc: %s, Expected path: %s, Got path: %s", tt.name, tt.wantSrcBinPath, gotSrcBinPath)
			}
			if tt.wantDeploymentDir != gotSrcDeploymentsDir {
				t.Errorf("Tc: %s, Expected path: %s, Got path: %s", tt.name, tt.wantDeploymentDir, gotSrcDeploymentsDir)
			}
			if !tt.wantErr == (err != nil) {
