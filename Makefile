...
generate-apiserver: ## Generate OpenAPISpec library based on ododevapispec.yaml inside pkg/apiserver-gen; also go fmt the files
	podman run --rm -v ${PWD}:/local openapitools/openapi-generator-cli generate -i /local/ododevapispec.yaml -g go-server -o /local/pkg/apiserver-gen --additional-properties=outputAsLibrary=true,onlyInterfaces=true && echo "Formatting generated files:" && go fmt ./pkg/apiserver-gen/...