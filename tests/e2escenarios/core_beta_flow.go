	...
	Context("when project from KUBECONFIG is used", func() {
		...
		Context("when component is in the current directory", func() {
			...
			It("'odo component' should fail if there already is .odo dir", func() {
				...
				helper.CmdShouldFail("odo", "component", "create", "nodejs")
			})
		...
		Context("when --context flag is used", func() {
			...
			It("odo component should fail if there already is .odo dir", func() {
				helper.CmdShouldPass("odo", "component", "create", "nodejs", "--context", context)
				helper.CmdShouldFail("odo", "component", "create", "nodejs", "--context", context)
			})
			...
			It("create local java component and push code", func() {
				helper.CopyExample(filepath.Join("source", "openjdk"), context)
				// Failing because odo config doesn't support --context
