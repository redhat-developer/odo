...
var _ = Describe("odo url command tests", func() {
	...
	Context("when listing urls using -o json flag", func() {
		...
		It("should be able to list url in machine readable json format", func() {
			...
			desiredURLListJSON := fmt.Sprintf(`{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[{"kind":"url","apiVersion":"odo.openshift.io/v1","metadata":{"name":"myurl","creationTimestamp":null},"spec":{"host":"%s","protocol":"http","port":8080,"secure":false},"status":{"state": "Pushed"}}]}`, pathNoHTTP)
		...
		It("should be able to list url in machine readable json format for a secure url", func() {
			...
			fullURLPath := helper.DetermineRouteURL("")
			pathNoHTTP := strings.Split(fullURLPath, "//")[1]
			desiredURLListJSON := fmt.Sprintf(`{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[{"kind":"url","apiVersion":"odo.openshift.io/v1","metadata":{"name":"myurl","creationTimestamp":null},"spec":{"host":"%s","protocol":"https","port":8080,"secure":true},"status":{"state": "Pushed"}}]}`, pathNoHTTP)
			Expect(desiredURLListJSON).Should(MatchJSON(actualURLListJSON))
		})
	})
