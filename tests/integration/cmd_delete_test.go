...
var _ = Describe("odo delete command tests", func() {
	...
	for _, withDotOdoDir := range []bool{true, false} {
		...
		When("using --files in a directory where Devfile was not generated by odo: withDotOdoDir="+strconv.FormatBool(withDotOdoDir), func() {
			var out string
			BeforeEach(func() {
				helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
			...
			It("should delete the relevant files", func() {
				...
				if withDotOdoDir {
					By("deleting the .odo directory", func() {
						Expect(fileList).ShouldNot(ContainElement(util.DotOdoDirectory))
					})
				}
				By("not deleting the devfile.yaml file", func() {
