...
var _ = Describe("odo devfile storage command tests", func() {
	...
	Context("When devfile storage delete command is executed", func() {
		It("should delete the storage and unmount it on the container", func() {
			...
			helper.CmdShouldPass("odo", "storage", "delete", storageNames[1], "-f", "--context", commonVar.Context)
			...
			pvcNames := commonVar.CliRunner.GetAllPVCNames(commonVar.Project)
		...
		It("should delete the storage and output in json format", func() {
			...
			helper.CmdShouldPass("odo", "storage", "create", "store-1", "--path", "/path", "--size", "1Gi", "--context", commonVar.Context)

			actualJSONStorage := helper.CmdShouldPass("odo", "storage", "delete", "store-1", "-f", "--context", commonVar.Context, "-o", "json")
