...
var _ = Describe("odo devfile watch command tests", func() {
	...
	Context("when executing odo watch after odo push", func() {
		It("should listen for file changes", func() {
			...
			utils.OdoWatch(utils.OdoV1Watch{}, odoV2Watch, globals.Project, globals.Context, watchFlag, cliRunner, "kube")
		})
	})
	...
	Context("when executing odo watch after odo push with custom commands", func() {
		It("should listen for file changes", func() {
			...
			output := helper.CmdShouldPass("odo", "push", "--build-command", "build", "--run-command", "run", "--devfile", "devfile.yaml", "--project", globals.Project)
			Expect(output).To(ContainSubstring("Changes successfully pushed to component"))
