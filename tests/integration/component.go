...
func componentTests(args ...string) {
	...
	Context("Creating component", func() {
		...
		It("should describe not pushed component when it is created with json output", func() {
			...
			Expect(err).Should(BeNil())
			sourcePath := helper.PlatformSpecificSourcePath()
			expected, err := helper.Unindented(`{"kind": "Component","apiVersion": "odo.dev/v1alpha1","metadata": {"name": "cmp-git","namespace": "` + project + `","creationTimestamp": null},"spec":{"app": "testing","type":"nodejs","source": "` + sourcePath + `","sourceType": "local","ports": ["8080/TCP"]}, "status": {"state": "Not Pushed"}}`)
			Expect(err).Should(BeNil())
			Expect(cmpDescribeJSON).To(Equal(expected))
			helper.CmdShouldPass("odo", append(args, "delete", "-f", "--all", "--context", context)...)
		})
