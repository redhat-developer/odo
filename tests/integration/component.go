...
func componentTests(args ...string) {
	...
	Context("convert s2i to devfile", func() {
		...
		JustBeforeEach(func() {
			project = helper.CreateRandProject()
			originalDir = helper.Getwd()
			// setting this till odo-init-image PR gets merged
			// https://github.com/openshift/odo-init-image/pull/70
			os.Setenv("ODO_BOOTSTRAPPER_IMAGE", "adisky/odo-init-image:7.0")
		})

		JustAfterEach(func() {
			helper.DeleteProject(project)
			helper.Chdir(originalDir)
			os.Unsetenv("ODO_BOOTSTRAPPER_IMAGE")
		})
