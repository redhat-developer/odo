...
func componentTests(args ...string) {
	...
	Context("Creating component", func() {
		...
		It("Without an application should create one", func() {
			...
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "--project", commonVar.Project, componentName, "--ref", "master", "--git", "https://github.com/openshift/nodejs-ex")...)
		...
		It("should create default named component when passed same context differently", func() {
			...
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "--context", ".", "--app", "testing")...)
			...
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "--context", commonVar.Context, "--app", "testing")...)
		...
		It("should show an error when ref flag is provided with sources except git", func() {
			outputErr := helper.CmdShouldFail("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "cmp-git", "--ref", "test")...)
		...
		It("create component twice fails from same directory", func() {
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "nodejs", "--project", commonVar.Project)...)
			output := helper.CmdShouldFail("odo", append(args, "create", "nodejs", "nodejs", "--project", commonVar.Project)...)
		...
		It("should list the component", func() {
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "cmp-git", "--project", commonVar.Project, "--git", "https://github.com/openshift/nodejs-ex", "--context", commonVar.Context, "--app", "testing")...)
			helper.ValidateLocalCmpExist(commonVar.Context, "Type,nodejs", "Name,cmp-git", "Application,testing")
		...
		It("should describe not pushed component when it is created with json output", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			cmpDescribeJSON, err := helper.Unindented(helper.CmdShouldPass("odo", append(args, "create", "nodejs", "cmp-git", "--project", commonVar.Project, "--context", commonVar.Context, "--app", "testing", "-o", "json")...))
			Expect(err).Should(BeNil())
		...
		It("should describe pushed component when it is created with json output", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			cmpDescribeJSON, err := helper.Unindented(helper.CmdShouldPass("odo", append(args, "create", "nodejs", "cmp-git", "--project", commonVar.Project, "--context", commonVar.Context, "--app", "testing", "-o", "json", "--now")...))
			Expect(err).Should(BeNil())
		...
		It("should list the component in the same app when one is pushed and the other one is not pushed", func() {
			...
			context2 := helper.CreateNewContext()
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "cmp-git-2", "--project", commonVar.Project, "--git", "https://github.com/openshift/nodejs-ex", "--context", context2, "--app", "testing")...)
	...
	Context("Test odo push with --source and --config flags", func() {
		...
		Context("Using project flag(--project) and current directory", func() {
			It("create local nodejs component and push source and code separately", func() {
				...
				helper.CmdShouldPass("odo", append(args, "create", "nodejs", cmpName, "--app", appName, "--project", commonVar.Project)...)
		...
		Context("when --context is used", func() {
			...
			It("create local nodejs component and push source and code separately", func() {
				...
				helper.CmdShouldPass("odo", append(args, "create", "nodejs", cmpName, "--context", commonVar.Context, "--app", appName, "--project", commonVar.Project)...)
				helper.ValidateLocalCmpExist(commonVar.Context, "Type,nodejs", "Name,"+cmpName, "Application,"+appName)
			...
			It("create local nodejs component and push source and code at once", func() {
				...
				helper.CmdShouldPass("odo", append(args, "create", "nodejs", cmpName, "--app", appName, "--context", commonVar.Context, "--project", commonVar.Project)...)
				helper.ValidateLocalCmpExist(commonVar.Context, "Type,nodejs", "Name,"+cmpName, "Application,"+appName)
	...
	Context("Test odo push with --now flag during creation", func() {
		...
		It("should successfully create config and push code in one create command with --now", func() {
			...
			cmpName := "nodejs-push-atonce"
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", cmpName, "--app", appName, "--project", commonVar.Project, "--now")...)
			helper.ValidateLocalCmpExist(commonVar.Context, "Type,nodejs", "Name,"+cmpName, "Application,"+appName)
	...
	Context("when component is in the current directory and --project flag is used", func() {
		...
		It("create local nodejs component twice and fail", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "--env", "key=value,key1=value1")...)
			output := helper.CmdShouldFail("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "--env", "key=value,key1=value1")...)
			Expect(output).To(ContainSubstring("this directory already contains a component"))
		})

		It("creates and pushes local nodejs component and then deletes --all", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", componentName, "--app", appName, "--project", commonVar.Project, "--env", "key=value,key1=value1")...)
		...
		It("creates a local python component, pushes it and then deletes it using --all flag", func() {
			...
			helper.CmdShouldPass("odo", append(args, "create", "python", componentName, "--app", appName, "--project", commonVar.Project, "--context", commonVar.Context)...)
		...
		It("creates a local python component, pushes it and then deletes it using --all flag in local directory", func() {
			helper.CopyExample(filepath.Join("source", "python"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "python", componentName, "--app", appName, "--project", commonVar.Project)...)
			helper.ValidateLocalCmpExist(commonVar.Context, "Type,python", "Name,"+componentName, "Application,"+appName)
			helper.CmdShouldPass("odo", append(args, "push")...)
		...
		It("creates a local python component and check for unsupported warning", func() {
			helper.CopyExample(filepath.Join("source", "python"), commonVar.Context)
			output := helper.CmdShouldPass("odo", append(args, "create", "python", componentName, "--app", appName, "--project", commonVar.Project, "--context", commonVar.Context)...)
		...
		It("creates a local nodejs component and check unsupported warning hasn't occured", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			output := helper.CmdShouldPass("odo", append(args, "create", "nodejs:latest", componentName, "--app", appName, "--project", commonVar.Project, "--context", commonVar.Context)...)
		...
		It("creates a local java component and check unsupported warning hasn't occured", func() {
			helper.CopyExample(filepath.Join("binary", "java", "openjdk"), commonVar.Context)
			output := helper.CmdShouldPass("odo", append(args, "create", "java:latest", componentName, "--project", commonVar.Project, "--context", commonVar.Context)...)
			Expect(output).NotTo(ContainSubstring("Warning"))
		})
	})
	...
	Context("odo component updating", func() {
		...
		It("should be able to create a git component and update it from local to git", func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "cmp-git", "--project", commonVar.Project, "--context", commonVar.Context, "--app", "testing")...)
			helper.CmdShouldPass("odo", append(args, "push", "--context", commonVar.Context)...)
	...
	Context("when running odo push multiple times, check for existence of environment variables", func() {
		It("should should retain the same environment variable on multiple push", func() {
			...
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", componentName, "--app", appName, "--project", commonVar.Project, "--context", commonVar.Context)...)
			helper.CmdShouldPass("odo", append(args, "push", "--context", commonVar.Context)...)
	...
	Context("Creating component with numeric named context", func() {
		...
		It("should create default named component in a directory with numeric name", func() {
			...
			helper.CmdShouldPass("odo", append(args, "create", "nodejs", "--project", commonVar.Project, "--context", contextNumeric, "--app", "testing")...)
