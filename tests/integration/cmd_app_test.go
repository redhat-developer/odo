...
var _ = Describe("odo app command tests", func() {
	...
	Context("when running app delete, describe and list command on fresh cluster", func() {
		It("should error out display the help", func() {
			...
			desired := `{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[]}`
	...
	Context("when running app command without app parameter in directory that contains .odo config directory", func() {
		...
		It("should successfully execute list, describe and delete along with machine readable output", func() {
			...
			desiredCompListJSON := fmt.Sprintf(`{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[{"kind":"Component","apiVersion":"odo.openshift.io/v1","metadata":{"name":"nodejs","creationTimestamp":null, "namespace":"%s"},"spec":{"type":"nodejs","app":"app","sourceType": "local","env":[{"name":"DEBUG_PORT","value":"5858"}]},"status":{"state":"Pushed"}}]}`, project)
			Expect(desiredCompListJSON).Should(MatchJSON(actualCompListJSON))

			helper.CmdShouldPass("odo", "app", "describe")
			desiredDesAppJSON := fmt.Sprintf(`{"kind":"Application","apiVersion":"odo.openshift.io/v1","metadata":{"name":"myapp","namespace":"%s","creationTimestamp":null},"spec":{}}`, project)
	...
	Context("when running app command app parameter in directory that doesn't contain .odo config directory", func() {
		It("should successfully execute list, describe and delete along with machine readable output", func() {
			...
			//desiredCompListJSON := `{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[]}`
			desiredCompListJSON := fmt.Sprintf(`{"kind":"List","apiVersion":"odo.openshift.io/v1","metadata":{},"items":[{"kind":"Application","apiVersion":"odo.openshift.io/v1","metadata":{"name":"app","namespace":"%s","creationTimestamp":null},"spec":{"components":["%s"]}}]}`, project, cmpName)
			Expect(desiredCompListJSON).Should(MatchJSON(actualCompListJSON))

			helper.CmdShouldPass("odo", "app", "describe", appName, "--project", project)
			desiredDesAppJSON := fmt.Sprintf(`{"kind":"Application","apiVersion":"odo.openshift.io/v1","metadata":{"name":"%s","namespace":"%s","creationTimestamp":null},"spec":{"components":["%s"]}}`, appName, project, cmpName)
