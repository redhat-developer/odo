...
var _ = Describe("odoURLIntegration", func() {
	...
	Context("Listing urls", func() {
		It("should list appropriate URLs and push message", func() {
			...
			Expect(stdout).To(ContainSubstring("To create URLS on the OpenShift Cluster, please use `odo push`"))
			helper.CmdShouldPass("odo", "push")
			stdout = helper.CmdShouldPass("odo", "url", "list")
			Expect(stdout).To(ContainSubstring(url1))
			Expect(stdout).ToNot(ContainSubstring("<not created on cluster>"))
			Expect(stdout).To(ContainSubstring("Present"))
			Expect(stdout).ToNot(ContainSubstring("To create URLS on the OpenShift Cluster, please use `odo push`"))
			helper.CmdShouldPass("odo", "url", "delete", url1, "-f")
			stdout = helper.CmdShouldPass("odo", "url", "list")
			Expect(stdout).To(ContainSubstring(url1))
			Expect(stdout).To(ContainSubstring("Absent"))
			Expect(stdout).To(ContainSubstring("To delete URLS on the OpenShift Cluster, please use `odo push`"))
			...
			Expect(stdout).To(ContainSubstring("Present"))
			Expect(stdout).To(ContainSubstring("To create/delete URLS on the OpenShift Cluster, please use `odo push`"))
			helper.CmdShouldPass("odo", "push")
			stdout = helper.CmdShouldPass("odo", "url", "list")
			...
			Expect(stdout).To(ContainSubstring(url2))
			Expect(stdout).To(ContainSubstring("Present"))
			Expect(stdout).ToNot(ContainSubstring("URLS on the OpenShift Cluster, please use `odo push`"))
		})
	})
})
