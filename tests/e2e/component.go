	...
	Context("Component tests", func() {
		...
		Context("Regression: adding and removing limits should reflect on DC", func() {
			It("should reflect the memory constraint in DC when updated in config", func() {
				...
				maxMemory := runCmdShouldPass(fmt.Sprintf("oc get dc -l app.kubernetes.io/instance=%s -o jsonpath='{%s.limits.memory}'", componentName, resourcePath))
				Expect(maxMemory).To(Equal("2Gi"))
				minMemory := runCmdShouldPass(fmt.Sprintf("oc get dc -l app.kubernetes.io/instance=%s -o jsonpath='{%s.requests.memory}'", componentName, resourcePath))
