...
func init() {
	...
	// Initialize a cryptographically-seeded to work around issue with seeding using time.Now().UnixNano(), see function for details.
	initSharedRand()
}
...
// UniqueRandString returns a random string of given length which is guaranteed
// to be random. This string is not vulnerable to collision due to shared math.rand seed values.
func UniqueRandString(n int) string {

	sharedRand.randMutex.Lock()
	defer sharedRand.randMutex.Unlock()

	const letterBytes = "abcdefghijklmnopqrstuvwxyz"
	b := make([]byte, n)
	for i := range b {
		b[i] = letterBytes[sharedRand.helperRand.Intn(len(letterBytes))]
	}
...
// initSharedRand initializes the 'cryptoInitializedRand' rand object using a seed generated by crypto.rand.
//
// Rather than using math.rand (which is vulnerable to generating the same numbers due to shared seeds),
// this seeds a math.rand() using a large truly random seed generated via the OS' crypto primitives.
func initSharedRand() {
	sharedRand.randMutex.Lock()
	defer sharedRand.randMutex.Unlock()

	if sharedRand.helperRand != nil {
		return
	}

	// Seed the pseudo-random number generator using a value from the crypto random number generator.
	nBig, err := crypto.Int(crypto.Reader, big.NewInt(math.MaxInt64))
	...
	sharedRand.helperRand = rand.New(src)
...
var sharedRand cryptoInitializedRand

// cryptoInitializedRand contains a rand object which has been initialized using a seed generated by crypto.rand; see initSharedRand() for details.
type cryptoInitializedRand struct {
	randMutex  sync.Mutex
	helperRand *rand.Rand
}
