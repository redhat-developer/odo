...
+
----
$ TEST_EXEC_NODES=1 make test-integration-all-service-catalog
----
+

----
$ TEST_EXEC_NODES=1 make test-integration-all
----
+

NOTE: `make test-integration-all-service-catalog` can be successfully validated only if the cluster has service catalog enabled. Whereas `make test-integration-all` runs the test irrespective of service catalog status in the cluster. `test-integration-all` includes all tests except service and link tests and `test-integration-all-service-catalog` includes only service and link tests.
