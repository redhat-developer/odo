...
* To run the integration test suite in parallel (default: 4 ginkgo test node), on a test cluster :
+
----
...
* To run the integration test suite sequentially or on single ginkgo test node use enviornment variable `TEST_EXEC_NODES`:
+
----
$ make test-integration TEST_EXEC_NODES=1
----
