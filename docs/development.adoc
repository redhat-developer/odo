...
* To run the template spec in parallel (default) on `4` test node :
+
----
$ make clean-test
----
...
* To run the template spec sequentially or on single test node use enviornment variable `TEST_EXEC_NODES`:
+
----
$ make clean-test SPEC_EXEC_METHOD=1
----

NOTE: Currently only the clean test template is configured with environment variable `TEST_EXEC_NODES` and rest of the test like `make test-cmp-e2e` and others does not honour this environment variable. So by default rest of the test runs sequencially.
