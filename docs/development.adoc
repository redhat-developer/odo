...
Refer odo clean test link:https://github.com/openshift/odo/blob/master/tests/template/template_cleantest_test.go[`template`].

.Procedure:

Integration tests can be run in the following two ways:

* To run component test spec in parallel (default) on `4` test node :
+
----
$ make test-cmp-e2e
----
...
* To run component test spec sequentially or on single test node use enviornment variable `TEST_EXEC_NODES`:
+
----
