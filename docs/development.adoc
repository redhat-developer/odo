...
NOTE: `make test-integration-service-catalog` includes only service and link tests and can be successfully validated only if the cluster has service catalog enabled. Whereas `make test-integration` includes tests except service and link tests and runs rest of the tests irrespective of service catalog status in the cluster.
