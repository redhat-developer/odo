...
* For the login and logout command integration test:
+
----
...
NOTE: `make test-odo-login-e2e` doesn't honour enviornment variable `TEST_EXEC_NODES`. So by default it runs login and logout command integration test suite on a single ginkgo test node sequentially to avoid race condition in parallel run.
