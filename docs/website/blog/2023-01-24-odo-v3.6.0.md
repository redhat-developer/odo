...
`pod-overrides` can be specified at both Component and Devfile attributes level while `container-overrides` can only be specified at Component level. If an attribute is defined at both the levels, a strategic merge patch will be used to merge them both.
