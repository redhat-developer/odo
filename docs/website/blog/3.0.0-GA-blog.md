...
Are you an application developer wanting to work closely with a production like environment? But can't, because you either don't have the time or energy to dive in the steep learning curve involved in running your application on the cluster?
...
### odo init
`odo init` allows you to fetch an appropriate [Devfile](https://devfile.io/docs/2.1.0/what-is-a-devfile) for your project.
...
_How is it different from `odo create` command of v2?_
* [Detect appropriate Devfile](#detect-appropriate-devfile)
...
_How is it different from `odo push` command of v2?_
...
#### Continuous workflow
`odo dev` is a non-terminating command that runs until user hits `Ctrl+c`. It continuously watches the directory for any new changes (including the changes occuring in the Devfile) and automatically syncs them with the application running on the cluster. 
...
Read this section on [What happened to Ingress/Route?](/docs/user-guides/v3-migration-guide#what-happened-to-ingressroute) to learn more.
...
_How is it different from `odo link` command of v2?_
...
As opposed to v2, `odo add binding` does not require for the application be running on the cluster, making it even faster.
...
You might find yourself wondering how `odo dev` is different from `odo deploy` and when should you use the either one, you can read [this article](https://odo.dev/docs/overview/dev_and_deploy) to get an answer to that question.
