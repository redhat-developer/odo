...
`odo` v3 comes with a new set of commands that replaces the old ones but essentially does the same thing, in a faster and efficient way.
...
`odo dev` uses port-forwarding to allow application access instead of Routes and Ingress that were used in `odo push`, making it much faster to access the application.
