...
At the time of this blog post, no Devfile within the default Devfile registry contains instructions for the deployment phase. By using the Devfile provided as example above, the command `odo deploy` will deploy a personalized Deployment and a Service into the cluster, after building the container image using the `Dockerfile` present in the directory.
