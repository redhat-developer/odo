...
| `ODO_PUSH_IMAGES`          | Whether to push the image once built; this is useful for integration tests running on podmanbecause images pushed by odo are created in a private registry and pulling them is not possible without setting up credentials to access it. `true` by default                                                                                                                     | v3.7.0        | `false`                         |
