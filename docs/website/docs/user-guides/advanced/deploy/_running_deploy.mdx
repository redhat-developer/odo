...
:::note
Depending upon the container registry which you specified in the `devfile.yaml`, the access for container image
pushed by `odo deploy` might be private by default, e.g., images pushed to [Quay.io](https://quay.io). Change the
