...
This is [inner loop](http://localhost:3000/docs/3.0.0/intro#what-is-inner-loop-and-outer-loop) development and allows you to code, build, run and test the application in a continuous workflow.
...
A much more descriptive explanation on each part of a Devefile can be found on the [Devfile API reference](https://devfile.io/docs/devfile/2.0.0/user-guide/api-reference/) site.
...
#### `starterProjects`
> The starter project that was initialized when running `odo init`.
```yaml
starterProjects:
- git:
    remotes:
...
>
> Each command contains what will be run in the command line as well as what corresponding group they are part of.
>
> The four groups are: build, run, test and debug.
>
> Build is what is initially ran when deploying to the cluster.
>
> Run executes the command when the container has been created.
>
> Debug is enabled when running `odo dev --debug`.
>
> Test is yet-to-be-implemented.
```yaml
commands:
- exec:
