...
Linking a component to a service means, in simplest terms, to make a service usable from the component. odo uses link:https://github.com/redhat-developer/service-binding-operator/tree/v0.5.0[Service Binding Operator v0.5.0] to provide the linking feature. Please refer link:install-service-binding-operator.adoc[this document] to install it on OpenShift or Kubernetes.
...
=== Linking services using files or environment variables

Link command by default will inject environment variables into your application.
Your application can reference environment variables directly in the codebase to connect to backing services.

Many frameworks support more secure and reliable ways of sharing credentials using file system.
link:https://github.com/redhat-developer/service-binding-operator/tree/v0.5.0[Service Binding Operator] provides ability to 
create file structure containing files representing values from the secret.
For example when executing `odo link` we can specify `bindAsFiles` option that will bind credentials into container filesystem.

[source,shell]
----
odo link --bindAsFiles
----

After pushing changes we could see number of files created inside the bindings folder
[source,shell]
----
ls /bindings/db
...
====
Number of frameworks like Quarkus, Node.js supports this way of loading credentials for popular databases and tools.
====
