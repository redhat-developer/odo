...
Vendor is important to make sure that odo can alway be build even offline.
...
+
NOTE:  If you want to use specific version of a module you can do `go get <pkg>@<version>`, for example (`go get golang.org/x/text@v0.3.2`)
+
...
. Create a blog post! Follow the https://github.com/openshift/odo/tree/master/docs/blog[template.md] file and push it to the website (a tutorial is located on the https://github.com/openshift/odo/tree/gh-pages[gh-pages] branch of odo
...
. Add the the built site (including the blog post) to the release with `site.tar.gz` https://github.com/openshift/odo/tree/gh-pages#bundling-the-site-for-releases[using the bundling the site for releases guide].
